...
def prefix_automaton(words, alphabet=None):
    ...
    return Automaton(transitions, initial_states=[frozenset([init])],
                     final_states=[frozenset([word]) for word in words],
                     alphabet=alphabet)
...
def rpni_simplify(automaton):
    ...
    while not A.is_deterministic():
        states_to_merge = list(filter(lambda e: len(e) > 1,
                                      A._transitions.values())).pop()
